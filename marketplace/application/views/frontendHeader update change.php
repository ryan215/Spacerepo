	...
	else
	{
		if(srchVal!='')
		{
			$.ajax({
				...
				data:'term='+srchVal+'&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>',
...
/*
	$('#sm_serachbox_pro378981551436342939').focusout(function(){
    	$('ul#autoSearchRes').html('');
		$('ul#autoSearchRes').css('display','none');
    });
*/});
      ...
      <div class="container">
        <div class="row">
          <div class="col-lg-2 col-md-2 col-sm-2 logo-wrapper">
            <h1> <a title="Shoppy Store" href="<?php echo base_url(); ?>"> <img src="<?php echo base_url(); ?>images/new_images/red/logo.png" alt="Shoppy Store" /> </a> </h1>
          </div>
...
<div class="mini-cart-header">
	<div class="block mini-cartpro sm-cartpro">
        ...
        <?php
		$totalCarts = $this->cart->contents();
		if(!empty($userId))
		{
			$cartDet = $this->cart_m->get_user_cart($userId);
			if((!empty($totalCarts))&&(count($totalCarts)>0))
			{
			?>
				<a href="<?php echo base_url().'frontend/product_cart/cart_page'; ?>" id="cartAnchor" title="Cart">
					<div class="info-mini-cart">
						<span><span id="totalProduct"><?php echo count($totalCarts); ?></span> Items </span>
					</div>
				</a>
			<?php
			}
			elseif((!empty($cartDet))&&(count($cartDet)>0))
			{
			?>
				<a href="<?php echo base_url().'frontend/product_cart/cart_page'; ?>" id="cartAnchor" title="Cart">
					<div class="info-mini-cart">
						<span><span id="totalProduct"><?php echo count($cartDet); ?></span> Items </span>
					</div>
				</a>
			<?php
			}
			...
			{
			?>
			<a href="javascript:void(0);" id="cartAnchor" title="Cart">
					<div class="info-mini-cart">
						<span><span id="totalProduct">0</span> Items </span>
					</div>
				</a>
			<?php
			}
		}
		elseif((!empty($totalCarts))&&(count($totalCarts)>0))
		{
		?>
			<a href="<?php echo base_url().'frontend/product_cart/cart_page'; ?>" id="cartAnchor" title="Cart">
				<div class="info-mini-cart">
                      ...
                      <!--End Module-->
										...
										<div class="search-pro col-lg-9 col-md-9 no-padding-l">
											<div id="sm_serachbox_pro378981551436342939" class="sm-serachbox-pro">
												<div class="sm-searbox-content">
													...
													<div class="form-search">
														<div class="input-search"> 
															<input type="text" class="typeahead tt-query search-text" autocomplete="off" spellcheck="false"  maxlength="70" name="all_search" id="search" onKeyPress="return not_submit(event,this.value);" onBlur="return not_submit(event,this.value);">
														</div>
												...
												<div>
													<ul id="autoSearchRes" class="ui-autocomplete ui-menu ui-widget ui-widget-content ui-corner-all" style="display:none; "></ul>
												</div>
											</div>
										</div>
                					</div>
