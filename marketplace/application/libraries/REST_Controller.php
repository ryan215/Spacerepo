...
abstract class REST_Controller extends MY_Controller
{
    ...
    public function __construct($config = 'rest')
    {
        ...
        if (!$this->input->is_ajax_request() and config_item('rest_ajax_only')) {
            $response = array(config_item('rest_status_field_name') => false, config_item('rest_message_field_name') =>array('message'=>'Only AJAX requests are accepted.')  );
    ...
    public function _remap($object_called, $arguments)
    {
        ...
        if (config_item('force_https') and !$this->_detect_ssl()) {
            $this->response(array(config_item('rest_status_field_name') => false, config_item('rest_message_field_name') =>array('message'=>'Unsupported protocol')  ), 403);
        }
        ...
        if (config_item('rest_enable_keys') and $use_key and $this->_allow === false) {
            ...
            $this->response(array(config_item('rest_status_field_name') => false, config_item('rest_message_field_name') => array('message'=>'Invalid API Key '.$this->rest->key) ), 403);
        }
        ...
        if (config_item('rest_enable_keys') and $use_key and !empty($this->rest->key) and !$this->_check_access()) {
            ...
            $this->response(array(config_item('rest_status_field_name') => false, config_item('rest_message_field_name') => array('message'=>'This API key does not have access to the requested controller.') ), 401);
        }
        // Sure it exists, but can they do anything with it?
        if ( ! method_exists($this, $controller_method)) {
            $this->response(array(config_item('rest_status_field_name') => false, config_item('rest_message_field_name') => array('message'=>'Unknown method.') ), 404);
        }
        ...
        if (config_item('rest_enable_keys') and !empty($this->rest->key)) {
            ...
            if (config_item('rest_enable_limits') and !$this->_check_limit($controller_method)) {
                $response = array(config_item('rest_status_field_name') => false, config_item('rest_message_field_name') => array('message'=>'This API key has reached the hourly limit for this method.') );
            ...
            $response = array(config_item('rest_status_field_name') => false, config_item('rest_message_field_name') => array('message'=>'This API key does not have enough permissions.') );
    ...
    protected function _perform_library_auth($username = '', $password = null)
    {
      ...
      $value=  $this->{$auth_library_class}->$auth_library_function($username, $password);
        $this->ci->custom_log->write_log('custom_log','security a1 value'.print_r($value.' /',true));
    ...
    protected function _prepare_digest_auth()
    {
        ...
        $this->ci->custom_log->write_log('custom_log',print_r($digest_string.' /',true));
        ...
        $this->ci->custom_log->write_log('custom_log','security a1 value'.print_r($A1.' /',true));

        if ( (! array_key_exists('username', $digest)) or ! $A1 ) {
            $this->_force_login($uniqid);
        }
        $A2 = md5(strtoupper($this->request->method).':'.$digest['uri']);
        $this->ci->custom_log->write_log('custom_log','security a2 value'.print_r($this->request->method.'/'.$A2.' /'.$digest['nonce'].'/'.$digest['nc'].'/'.$digest['cnonce'].'/'.$digest['qop'],true));
        $valid_response = md5($A1.':'.$digest['nonce'].':'.$digest['nc'].':'.$digest['cnonce'].':'.$digest['qop'].':'.$A2);
        $this->ci->custom_log->write_log('custom_log','security validresponse value'.print_r($digest['response'].' /'.$valid_response,true));
        if ($digest['response'] != $valid_response) {
           $this->response(array(config_item('rest_status_field_name') => 0, config_item('rest_message_field_name') =>array('message'=>'Invalid credentials') ), 401);
            exit;
        }
    }
    /**
    ...
    protected function _check_whitelist_auth()
    {
        ...
        if ( ! in_array($this->input->ip_address(), $whitelist)) {
            $this->response(array(config_item('rest_status_field_name') => false, config_item('rest_message_field_name') => array('message'=>'IP not authorized')), 401);
        }
    }
    /**
     * @todo Document this.
    ...
    protected function _force_login($nonce = '')
    {
        ...
        $this->response(array(config_item('rest_status_field_name') => false, config_item('rest_message_field_name') => array('message'=>'Not authorized') ), 401);
    }
    /**
