...
class Retailer_management extends MY_Controller {
	...
	public function index()
	{
		...
		$this->adminCustomView('retailer_managements/retailer_list',$this->data);
	}
	...
	public function ajaxFun($total=0)
	{
		...
		$this->load->view('retailer_managements/ajaxPagView',$this->data);
	}
	...
	public function addRetailer()
	{
		...
		$this->adminCustomView('retailer_managements/addRetailer',$this->data);	
	}
	...
	public function editRetailer($organizationId=0)
	{
		...
		$this->adminCustomView('retailer_managements/editRetailer',$this->data);		
	}
	...
	public function user_detail($organizationId)
	{
		...
		//echo $this->db->last_query(); exit;
		$this->adminCustomView('retailer_managements/user_details',$this->data);
	}
	...
	public function unblock_block($orgnizationId=0,$employeeId,$status)
	{
		...
		redirect(base_url().'admin/retailer_management/user_detail/'.$orgnizationId);
	}
	...
	public function assign_cse($organizationId)
	{
		$this->session->set_userdata(array(
		...
		$organizationId 			  = id_decrypt($organizationId);
		$this->data['result']     = $this->profile_lib->addEditAssignCSE($organizationId);		
		$this->data['organizationId'] = $organizationId;
	...
	public function request_list()
	{
		...
		$this->adminCustomView('retailer_managements/request_list',$this->data);
	}
	...
	public function requestAjaxFun($total=0)
	{
		...
		$this->load->view('retailer_managements/requestAjaxFun',$this->data);
	}
	
	public function request_user_detail($organizationId)
	{
		$this->session->set_userdata(array(
		...
		$organizationId = id_decrypt($organizationId);
		$result = $this->profile_lib->retailer_RequestUserDetails($organizationId);
		...
		if($_POST)
		{
			...
			{
				if($this->user_m->decline_retailer_request($result['employeeId']))
				{
		...
		$this->data['result'] = $result;
		$this->data['organizationId'] = $organizationId;
		$this->adminCustomView('retailer_managements/request_user_detail',$this->data);
	}
	
	public function accept_request($organizationId)
	{
		$this->session->set_userdata(array(
		...
		$organizationId = id_decrypt($organizationId);
		$this->user_m->update_dropship($organizationId,$this->input->post('dropship'));
		
		if(!empty($organizationId))
		{
			$details = $this->user_m->getEmployeeRetailer($organizationId);
			//echo "<pre>"; print_r($details); exit;
			if(!empty($details))
			{
				if($this->user_m->accept_retailer_request($details->employeeId))
				{
				//echo $this->db->last_query(); exit;
					...
					else
					{
						if(!empty($emailId))
						{
							...
							$message = 'Your account has been approved to sell on pointemart. All you need to do is  choose products you want to sell from our product catalogue, Set the price and inventory. For any queries please call us at '.$this->config->item('admin_phone_no').' or email us at '.$this->config->item('admin_email');
 							$rs      = $this->twillo_m->send_mobile_message($countryCOde.$businessPh,$message);
					}
			...
			else
			{
				...
				redirect(base_url().'admin/retailer_management/request_user_detail/'.id_encrypt($organizationId));
			}
		}
		redirect(base_url().$this->session->userdata('userType').'/retailer_management/request_list');		
	}
	
	public function editRetailerRequest($organizationId=0)
	{
		$this->session->set_userdata(array(
	...
	public function request_assign_cse($organizationId)
	{
		$this->session->set_userdata(array(
				'log_MODULE' => 'assign_cse',
				'log_MID'    => '' 
	...
	public function request_history_list()
	{
		...
		$this->adminCustomView('retailer_managements/request_history_list',$this->data);
	}
	...
	public function requestHistoryAjaxFun($total=0)
	{
		...
		$this->load->view('retailer_managements/requestHistoryAjaxFun',$this->data);
	}
	
	public function request_history_user_detail($organizationId)
	{
		$this->session->set_userdata(array(
		...
		$organizationId 		  = id_decrypt($organizationId);
		$this->data['result'] = $this->profile_lib->retailer_RequestUserDetails($organizationId);
		$this->adminCustomView('retailer_managements/request_history_user_detail',$this->data);
	}
}