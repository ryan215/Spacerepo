...
class Vendor_management extends MY_Controller {
	...
	public function index()
	{
		...
		$this->adminCustomView('vendor_managements/vendore_list',$this->data);
	}
	...
	public function ajaxFun($total=0)
	{
		...
		$this->data['result'] = $this->vendor_lib->shipping_vendor_ajaxFun();
		$this->load->view('vendor_managements/ajaxPagView',$this->data);
	}
	...
	public function addVendore()
	{
		...
		$result 					   = $this->vendor_lib->shipping_vendore_sign_up();
		...
		$this->adminCustomView('vendor_managements/addVendore',$this->data);	
	}
	
	public function check_unique_vendor_user()
	{
		$email = trim($this->input->post('email'));
		if(!empty($email))
		{
			$details = $this->shipping_m->check_unique_vendor_user($email);
			if(!empty($details))
			{
				$this->form_validation->set_message('check_unique_vendor_user','Your email is already registered on PointeMart. Please use another email address to register on Vendor');
				return FALSE;
			}
			else
			{
		...
		{
		 	$this->form_validation->set_message('unique_shipping_vendor_user', 'The %s field is required.');
			return FALSE;
		}
	}
	...
	public function user_detail($organizationId)
	{
		...
		$this->data['result']         = $this->vendor_lib->shipping_vendor_userDetails($organizationId);
		$this->data['organizationId'] = $organizationId;
		$this->adminCustomView('vendor_managements/user_details',$this->data);
	}
	...
	public function editVendor($organizationId=0)
	{
		...
		$organizationId 			   = id_decrypt($organizationId);
		$this->data['result'] 		   = $this->vendor_lib->shipping_vendor_edit($organizationId);
		$this->data['imagePath'] 	   = base_url().'uploads/shipping/thumb50/';
		$this->data['imageUploadPath'] = base_url().'admin/vendor_management/upload_image/';
		$this->data['organizationId']  = $organizationId;
		$this->adminCustomView('vendor_managements/editVendor',$this->data);		
	}
	...
	public function businessPhone()
	{
		...
		$details       = $this->shipping_m->check_business_phone_exits($employeeId,$businessPhone);
		if(!empty($details))
		{
	...
	public function email_exits()
	{
		...
		if(!empty($email))
		{
			$details    = $this->shipping_m->check_email_exits($employeeId,$email);
			if(!empty($details))
			{
	...
	public function unblock_block($organizationId,$employeeId,$status)
	{
		...
		if(!empty($employeeId))
		{
			$return = $this->vendor_lib->block_unblock($employeeId,$status);
			if($return)
			{
				if(!$status)
				{
	...
	public function upload_image()
	{
		...
		$imageName = $this->vendor_lib->upload_shipping_vendor_image();
		echo $imageName;
	}
	...
	public function check_businessPhone()
	{
		...
		$businessPhone = $this->input->post('businessPhone'); 
		$details       = $this->shipping_m->check_businessPhone($businessPhone,$countryCode,$employeeId);
	...
	public function upload_document_excel()
	{
		...
		$result = $this->vendor_lib->shipping_vendor_upload_document();
	...
	{	
		...
		if($this->shipping_m->delete_shipping_rate($shippingRateId))
		{
	...
	public function addShippingRates($shippingOrgId=0)
	{
		...
		if(!empty($shippingOrgId))
		{
			$this->vendor_lib->add_shipping_rates($shippingOrgId);
			$this->data['shippingOrgId'] = $shippingOrgId;
			$this->adminCustomView('vendor_managements/addShippingRates',$this->data);	
		}
	}
	...
	public function edit_rate($shippingRateId=0)
	{
		...
		$shippingRateId 			   = id_decrypt($shippingRateId);
		$this->data['result'] 		   = $this->vendor_lib->shipping_vendor_edit_rate($shippingRateId);
		$this->adminCustomView('vendor_managements/editRate',$this->data);		
	}
	...
	public function shipping_rate_list($shippingOrgId)
	{
		...
		$this->data['result'] = $this->vendor_lib->shipping_rates_list($shippingOrgId);
		$this->data['shippingOrgId'] = $shippingOrgId;
		$this->adminCustomView('vendor_managements/shipping_rates_list',$this->data);
	}
	...
	public function shippingRatesAjaxFun($total=0)
	{
		...
		$this->data['result'] = $this->vendor_lib->shipping_rates_list_ajax($total);
		$this->load->view('vendor_managements/shipping_rates_list_ajax',$this->data);	
	}
	...
	public function addShippingRatesFrom10To30($shippingOrgId)
	{
		...
		$shippingOrgId 		 = id_decrypt($shippingOrgId);		
		$this->data['title'] = 'Shipping Rate List';
		$this->data['result'] = $this->vendor_lib->add_shipping_rates_from_10_to_30($shippingOrgId);
		$this->data['shippingOrgId'] = $shippingOrgId;
		$this->adminCustomView('vendor_managements/addShippingRatesFrom10To30',$this->data);
	}
	
	public function add_shipping_rate($shippingOrgId=0)
	{
		$shippingOrgId  = id_decrypt($shippingOrgId);
		
		if(!empty($shippingOrgId))
		{
			$this->data['result']		 = $this->vendor_lib->add_shipping_rate($shippingOrgId);
			$this->data['shippingOrgId'] = $shippingOrgId;
			$this->adminCustomView('vendor_managements/add_shipping_rate',$this->data);	
		}
	}
	
	public function add_shipping_rate_list($shippingOrgId=0,$shipRateStepFirstId=0)
	{
		$shippingOrgId  	 = id_decrypt($shippingOrgId);
		$shipRateStepFirstId = id_decrypt($shipRateStepFirstId);
		
		if((!empty($shippingOrgId))&&($shipRateStepFirstId))
		{
			$this->data['result']		 = $this->vendor_lib->add_shipping_rate_list($shippingOrgId,$shipRateStepFirstId);
			$this->data['shippingOrgId'] = $shippingOrgId;
			$this->data['shipRateStepFirstId'] = $shipRateStepFirstId;
			$this->adminCustomView('vendor_managements/add_shipping_rate_list',$this->data);	
		}
	}
	...
	public function area_list()
	{
		...
		$this->data['areaErr']  = $this->input->post('areaErr');
		echo $this->load->view('vendor_managements/area_list',$this->data,true);		
	}
	
	public function test1()
	{
		$this->data['title'] = 'Test1';
		$this->adminCustomView('vendor_managements/test1',$this->data);
	}
	
	public function test2()
	{
		$this->data['title'] = 'Test2';
		$this->adminCustomView('vendor_managements/test2',$this->data);
	}
