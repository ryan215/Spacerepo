					...
					<div class="panel-body">
						<section class="panel custom-panel">
							...
							<div style="max-width:700px; margin:0 auto; padding-bottom:25px;">
								<Center>
									<img src="<?php echo base_url().'img/2.png'; ?>" class="img-responsive" style="display: inline-block;">
								</center>
							</div>
							<?php
							}
...
{
	ajaxPage('<?php echo base_url().$this->session->userdata('userType').'/confirm_order/confirmationOrderAjaxFun/'.$result['total']; ?>');
}
...
function ready_to_shipped()
{
	...
	function(isConfirm){   
		...
		{   
			$.ajax({
				type: "POST",
				url:'<?php echo base_url().$this->session->userdata('userType').'/confirm_order/change_new_order_to_ready_bulk/'; ?>',
				data:$('.chkbx').serialize(),
				dataType:'json',
				success:function(result){
					...
					{
						window.location.href = '<?php echo base_url().$this->session->userdata('userType').'/confirm_order'; ?>';
					}
				}
			});
		} 
