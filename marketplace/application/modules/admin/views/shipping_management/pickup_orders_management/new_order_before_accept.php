...
function accept_decline(order_id,acpt_decl)
{
	...
	{
		window.location.href = '<?php echo base_url().'retailer/new_pickup_order/accept_decline/';?>'+order_id+'/'+acpt_decl;
	}
	return false;
}
/*function send_to_other(order_id)
{
	...
	{
		window.location.href = '<?php //echo base_url().'admin/new_pickup_order/send_to_other_retailers/'; ?>'+order_id;
	}
	return false;
}*/
...
{
	ajaxPage('<?php echo base_url().$this->session->userdata('userType').'/new_pickup_order/new_order_ajax/'.$result['total']; ?>');
}
