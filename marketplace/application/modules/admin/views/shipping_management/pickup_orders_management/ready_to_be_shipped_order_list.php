		...
		<div class="row">
			<div class="col-md-12">
				<ul class="breadcrumbs-alt">
					<li>
						<a href="javascript:void(0);" class="current">Ready To Pickup</a>
					</li>
				</ul>
			</div>
						...
						<section class="panel custom-panel">
							<div class="col-lg-12" style="padding:0;">
								...
								<div class="col-sm-7 " style="padding: 5px;">
									<!--<button type="button" onclick="generate_manifesto()" class="btn btn-sm btn-shadow btn-success hvr-push">Generate Manifesto</button>-->
								</div>
							</div>
							<?php
							...
							<div style="max-width:500px; margin:0 auto; padding-bottom:25px;">
								<Center>
									<img src="<?php echo base_url().'images/ready_pick.png'; ?>" class="img-responsive" style="display: inline-block;">
								</center>
							</div>
...
function generate_manifesto()
{
	$.ajax({
		type: "POST",
		url:'<?php echo base_url().$this->session->userdata('userType').'/ready_to_pickup/manifestolist'; ?>',
...
{
	ajaxPage('<?php echo base_url().$this->session->userdata('userType').'/ready_to_pickup/readyShippedAjaxFun/'.$result['total']; ?>');
}
...
function ready_to_shipped()
{
	...
	function(isConfirm){   
		...
		{   
			$.ajax({
				type: "POST",
				url:'<?php echo base_url().$this->session->userdata('userType').'/ready_to_pickup/change_status_bulk/'; ?>',
				...
				success:function(result){
					...
					{
						window.location.href = '<?php echo base_url().$this->session->userdata('userType').'/ready_to_pickup'; ?>';
					}
				}
			});
		} 
