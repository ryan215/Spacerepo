        ...
        <div class="row">
            <div class="col-md-12">
                <ul class="breadcrumbs-alt animated fadeInLeft">
                    ...
                    <li>
                        <a href="javascript:void(0);" class="">Marketing Product List</a>
                    </li>
                    <li>
            ...
            <div class="col-lg-12">
                <section class="panel">
                    ...
                    <?php 
					$attributes = array('id' => 'marketingFrm');
                                                ...
                                                <div class="row">
															...
															<div class="col-lg-6" style="padding-bottom:10px;">
																	  ...
																	  <label>Flash sale Inventory</label><span class="error">*</span>
																		  <div class="iconic-input ">
																			  <i class="fa fa"></i> 
																			  <input type="number" max="<?php echo $inventorydetail->currentQty;?>" class="form-control" name="inventory" min="0" value="<?php echo $inventory_details; ?>" placeholder="">
...
{	
	...
	<div class="col-lg-6" style="padding-bottom:10px;">
		<label>Inventory Details</label>
			...
			<i class="fa fa"></i> 
			<input type="number" max="<?php echo $currentStock; ?>"class="form-control" name="<?php echo 'inventoryarr_'.$key.preg_replace('/[^A-Za-z0-9\-]/','',$size['size']); ?>" value="<?php if(isset($inventory_details[$key.$size['size']])){ echo $inventory_details[$key.$size['size']]; } ?>" placeholder="" min="0" >
...
?>
																	  ...
																	  <label>Effective sales price</label><span class="error">*</span>
																		  <div class="iconic-input ">
																			  <i class="fa fa">₦</i> 
																			  <input type="text" class="form-control" onkeyup="set_discount(this.value)" onblur="set_discount(this.value)" name="sale" value="<?php echo $result['sale'];?>" placeholder="">
																		  </div>
																	  ...
																	  <label>Discount</label><span class="error">*</span>
																		  <div class="iconic-input ">
																			  <i class="fa fa">₦</i> 
																			 <input type="text" readonly class="form-control discount" name="discount" value="<?php echo $result['discountPrice'];?>" placeholder="">
																		  </div>
																	  ...
																	  <label>Discount pay by spacepointe</label><span class="error">*</span>
																		  <div class="iconic-input ">
																			  <i class="fa fa">₦</i> 
																			 <input type="text"  min="0" class="form-control spacepointediscount" name="spacepointediscount" id="spacepointediscount" onkeyup="set_retailer_amount(this.value);" onblur="set_retailer_amount(this.value);" value="<?php echo $result['spacepointeDiscount'];?>" placeholder="">
																		  </div>
																	  ...
																	  <label>Discount pay by retailer</label><span class="error">*</span>
																		  <div class="iconic-input ">
																			  <i class="fa fa">₦</i> 
																			 <input type="text" min="0"  class="form-control retailerdiscount" onkeyup="set_spacepointe_amount(this.value);" onblur="set_spacepointe_amount(this.value);" name="retailerdiscount" id="retailerdiscount" value="<?php echo $result['retailerDiscount'];?>" placeholder="">
																		  </div>
    ...
    <input type="text" class="form-control default-date-picker" name="datefrom" value="<?php echo $result['datefrom']; ?>" >
	<div class="error" ><?php echo form_error('datefrom'); ?></div>
</div>
	<div class="col-lg-6" style="padding-bottom:10px;">
		<label>Effective Date To</label><span class="error">*</span>
		<input type="text" class="form-control default-date-picker" name="dateto" id="dateto" value="<?php echo $result['dateto']; ?>" onblur="save_changes();">
                                                            ...
                                                            <select class="chosen-select form-control"
                                                                ...
                                                                if (!empty($catlist)) {
                                                                    foreach ($catlist as $row) {
                                                                        ?>
                                                                        <option
                                                                            value="<?php echo $row->categoryId; ?>" <?php if($result['category_listing'][0]==$row->categoryId){ echo 'selected="selected"'; } ?> >
...
{ 
	setTimeout(function(){
  		toDt = $('#dateto').val();
		toDayDt = '<?php echo date('Y-m-d'); ?>';
		//msg = 'Are you sure You want to save this changes?';
		if((toDt!='')&&(toDt<toDayDt))
		{
			msg = "The date you have selected is prior to today date, this will remove the product from the listing. Click ok to confirm.";
    ...
    function cat_list(segmntID, level,categoryId) {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url().$this->session->userdata('userType'); ?>/category_management/sementics_category_list/' + segmntID + '/' + level +'/'+categoryId,
 ...
 foreach($result['category_listing'] as $key=>$category)
{
	?> cat_list('<?php echo $category;?>','<?php echo $key+1; ?>' <?php if(isset($result['category_listing'][$key + 1])) { echo ','.$result['category_listing'][$key + 1]; } ?> );
	<?php 
}
?>
	})
