    ...
    <div class="row">
      <div class="col-lg-12">
        <ul class="breadcrumbs-alt animated fadeInLeft">
          <li> <a href="javascript:void(0);" class="current">City List</a> </li>
        </ul>
      </div>
      <div class="col-lg-12">
        <section class="panel">
...
{
?>
				  <div class="col-sm-2 col-lg-2" style="padding-left:0px;"> <a href="<?php echo base_url().$this->session->userdata('userType').'/location_management/addEditZip'; ?>" class="btn btn-sm btn-shadow btn-success hvr-push"> <i class="fa fa-plus"></i> Add </a> </div>
				  <?php
				  }
				  ?>
              ...
              <table class="table table-invoice table-hover table-striped table-custom table-search-head">
                <thead>
                  <tr>
                    ...
                    <th width="20%"> City
                      <input type="text" class="form-control search table-head-search" id="cityName" onkeyup="ajax_search();" placeholder="Zip">
                    </th>
					<?php
					$userRole = $this->session->userdata ('userRole');
if(empty($userRole))
...
function ajax_search(srchval)
{
	...
	postData = 'sel_no_entry='+$('#sel_no_entry').val()+'&countryName='+$("#countryName").val()+'&stateName='+$("#stateName").val()+'&areaName='+$("#areaName").val()+'&cityName='+$("#cityName").val();
	ajax_function('<?php echo base_url().$this->session->userdata('userType').'/location_management/zip_ajaxFun/'.$result['total']; ?>','#ajaxData',postData);
}
...
ajaxPage('<?php echo base_url().$this->session->userdata('userType').'/location_management/zip_ajaxFun/'.$result['total']; ?>');
</script>