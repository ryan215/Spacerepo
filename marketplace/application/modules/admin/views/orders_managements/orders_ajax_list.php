...
<tr>
		...
		<table width="100%" class="table table-striped   stock_table cus_table">
			...
			<tr>
                ...
                <td>
					...
					{
						if($row->isEconomicDelivery==2)
						{
							echo 'Same Day Delivery';
						}
						elseif($row->isEconomicDelivery==1)
						{
		...
		<table width="100%" class="table table-striped   stock_table cus_table">
			...
			<tr>
                ...
                <td>
					...
					if($this->session->userdata('userType')!='retailer')
					{
						echo '&#x20A6;'.number_format($row->totalCustomAmount,2); 
					}
					else
					{
						echo '&#x20A6;'.number_format($row->totalRetailerAmount,2);					
					}
					?>
                </td>
            </tr>
    ...
    <td>
		<center>
			...
			{
			?>
				<a href="<?php echo base_url().$this->session->userdata('userType').'/orders_management/pickup_order_view/'.id_encrypt($row->orderDetailId); ?>" class="btn btn-warning btn-xs" type="button" title="View Detail">
						<i class="fa fa-eye"></i>
				</a>
			<?php
			}
			...
			{
				if($row->isEconomicDelivery==1)
				{
				?>
					<a href="<?php echo base_url().$this->session->userdata('userType').'/orders_management/single_shippment_order_view/'.id_encrypt($row->orderCustomPaymentId); ?>" class="btn btn-warning btn-xs" type="button" title="View Detail">
						<i class="fa fa-eye"></i>
					</a>
				<?php
				}
				...
				{
				?>
					<a href="<?php echo base_url().$this->session->userdata('userType').'/orders_management/quick_shippment_order_view/'.id_encrypt($row->orderDetailId); ?>" class="btn btn-warning btn-xs" type="button" title="View Detail">
						<i class="fa fa-eye"></i>
					</a>
			<?php
				}
