...
.confirm{margin: 35px auto !important;
}
                	...
                	<div class="panel-body" style="padding:0px;">
						<section class="panel custom-panel">
                        	<div class="col-lg-12" style="padding:0;">
                            	<header class="panel-heading panel-heading1">Edit Product Type </header>						                      
                        	</div>
                        </section>
						<?php 
						...
						<section id="unseen">
							...
							<div class="col-sm-12 ">
                        		  <div class="form-group">
										...
										<div class="col-sm-4 padding_left_zero" style="padding-top:6px; font-size:16px;">
...
{
	swal({   title: "",   text: "If you delete the attribute type then all the attributes names under it will also be deleted. Are you want to proceed ?",   type: "warning",   showCancelButton: true,   confirmButtonColor: "#DD6B55",   confirmButtonText: "Yes",   closeOnConfirm: false }, 
	function(){   swal("Deleted!", "", "success"); 
...
{	
	ttlbx = 0;
	flag = true;
	$('#attrNameLst'+attrTyID).find('input[type="text"]').each(function(){
		if($(this).val()=='')
		{
			ttlbx++;
		}
...
{	
	...
	if(flag)
	{
		...
		if(childCntr)
		{
			rmvBtn = '<a href="javascript:void(0);" onclick="return removeAttributeTp('+childCntr+');" class="btn btn-danger btn-xs"><i class="fa fa-trash-o "></i></a>';
		}
...
?>
	...
	cntr1 = 100;
	<?php
	foreach($result['attrList'] as $key=>$value)
	{
?>
	rmvBtn = '<a href="javascript:void(0);" onclick="return removeAttributeTp('+cntr+');" class="btn btn-danger btn-xs"><i class="fa fa-trash-o "></i></a>';
	html = '<div class="form-group" id="attrTp'+cntr+'"><div class="col-sm-2 padding_left_zero"><label for="attribute_type" style="float:left; line-height:33px; margin-right:10px;">Attribute Type</label></div><div class="col-sm-4 padding_left_zero"><input type="text" name="attribute_type['+cntr+']" class="form-control" value="<?php echo $value['attribute_type']; ?>"></div><div class="col-sm-1 padding_left_zero plus-delete-cion">'+rmvBtn+'</div></div><div class="clearfix"></div><br /><table  class="table table-invoice table-custom atribt-table" id="attrNmTable'+cntr+'"><thead><tr><th>Attribute Name</th><th>Mandatory</th><th>Display On Frontend</th><th>Key Features</th><th width="8%"></th></tr></thead><tbody id="attrNameLst'+cntr+'">';
		html+= '<tr id="attrNmTr'+cntr1+'"><td><input type="text" class="form-control" name="attribute_name['+cntr+']['+cntr1+']" value="" placeholder="Enter Attribute Name" /></td><td><div class="checkbox c-checkbox"><label><input type="checkbox"  class="is_required" id="isReq'+cntr+'_'+cntr1+'" name="is_required['+cntr+']['+cntr1+']"><span class="fa fa-check"></span></label></div></td><td><div class="checkbox c-checkbox"><label><input type="checkbox" id="isDespl'+cntr+'_'+cntr1+'" class="is_display" name="is_display['+cntr+']['+cntr1+']"  checked="checked"><span class="fa fa-check"></span></label></div></td><td><div class="checkbox c-checkbox"><label><input type="checkbox" id="keyFtrs'+cntr+'_'+cntr1+'" class="key_features" name="key_features['+cntr+']['+cntr1+']" onclick="key_check('+cntr1+','+cntr+');"><span class="fa fa-check"></span></label></div></td><td><a id="anchAdd'+cntr1+'" onclick="addAttributeName('+cntr1+','+cntr+');" class="btn btn-success btn-xs" style=""><i class="fa fa-plus"></i></a><a style="display:none;" id="anchRmv'+cntr1+'" href="javascript:void(0);" onclick="removeAttributeName('+cntr1+');" class="btn btn-danger btn-xs"><i class="fa fa-trash-o "></i></a></td></tr>';
		cntr1++;
		<?php
		$i = 1;
		foreach($value['attribute_name'] as $key1=>$value1)
		{				
		?>
			html+= '<tr id="attrNmTr'+cntr1+'"><td><input type="text" class="form-control" name="attribute_name['+cntr+']['+cntr1+']" value="<?php echo $value1['attribute_name']; ?>" placeholder="Enter Attribute Name" /></td><td><div class="checkbox c-checkbox"><label><input type="checkbox"  class="is_required" id="isReq'+cntr+'_'+cntr1+'" name="is_required['+cntr+']['+cntr1+']" <?php if($value1['isRequired']){ ?> checked="checked" readonly="readonly" <?php } ?>><span class="fa fa-check"></span></label></div></td><td><div class="checkbox c-checkbox"><label><input type="checkbox" id="isDespl'+cntr+'_'+cntr1+'" class="is_display" name="is_display['+cntr+']['+cntr1+']" <?php if($value1['isDisplayed']){ ?> checked="checked"  readonly="readonly" <?php } ?>><span class="fa fa-check"></span></label></div></td><td><div class="checkbox c-checkbox"><label><input type="checkbox" id="keyFtrs'+cntr+'_'+cntr1+'" class="key_features" name="key_features['+cntr+']['+cntr1+']" <?php if($value1['keyFeatures']){ ?> checked="checked" <?php } ?> onclick="key_check('+cntr1+','+cntr+');"><span class="fa fa-check"></span></label></div></td><td><a  style="display:none;"  id="anchAdd'+cntr1+'" onclick="addAttributeName('+cntr1+','+cntr+');" class="btn btn-success btn-xs" style=""><i class="fa fa-plus"></i></a><a  id="anchRmv'+cntr1+'" href="javascript:void(0);" onclick="removeAttributeName('+cntr1+');" class="btn btn-danger btn-xs"><i class="fa fa-trash-o "></i></a></td></tr>';
			cntr1++;
		<?php
			$i++;
		}
