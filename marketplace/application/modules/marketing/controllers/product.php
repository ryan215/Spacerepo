...
class Product extends MY_Controller {
	...
	public function product_list_grid()
	{
		$this->session->set_userdata(array(
		...
		$maxPrice      = 0;
		$productList   = array();
		$details       = $this->product_marketing_m->front_product_listing_test('','',$where);	
		...
		if(!empty($details))
		{
			...
			foreach($details as $row)
			{
				...
				else
				{
					...
					$productList[$row->productId]['productName']  = $row->productCodeOveride;
					...
					if($i==0)
					{
						$minPrice = $row->adminPrice;
					}
					$maxPrice = $row->adminPrice;
					$i++;
		...
		$this->data['categoryName'] = 'Flash Sales';
	...
	public function ajaxFunGrid($total=0)
	{
		...
		$totalData   = $this->product_marketing_m->front_product_listing_test('','',$where);
		$details     = $this->product_marketing_m->front_product_listing_test($page_number,48,$where,$pSorting);	
		if(!empty($details))
		{
			foreach($details as $row)
			{
				...
				else
				{
					...
					{
						$prdCatLst = $this->segment_cat_m->category_parent_list($row->mainCatId);
