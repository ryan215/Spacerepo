...
class Category extends MY_Controller {
	...
	public function ajaxFun($total=0)
	{
		...
		if(!empty($search))
		{
			if($sorting=='segment_name')
			{
				$where = " AND segment.segment_name LIKE '%".$search."%'  ";
			}
			elseif($sorting=='category_name')
			{
				$where = " AND category.category_name LIKE '%".$search."%'  ";
			}
			else
			{
				$where = " WHERE 
							(
								segment.segment_name LIKE '%".$search."%' 
							OR
								category.category_name LIKE '%".$search."%'
					 		)
							";		
			}	
			$total = $this->segment_cat_m->total_category_where($where);
		}
		...
		$this->data['category_list'] = $this->segment_cat_m->category_where($page,$pagConfig['per_page'],$where);
	...
	{
		$segmentID = $this->input->post('segment_id');
		$cat_name  = $this->input->post('category_name');
		
		$res = $this->segment_cat_m->check_cat_name($segmentID,$cat_name);
		if(!empty($res))
		{
