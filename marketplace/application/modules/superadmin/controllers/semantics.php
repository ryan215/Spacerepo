...
class Semantics extends MY_Controller {
	...
	public function index()
	{
		...
		$this->superAdminCustomView('admin/semantics/product_listing',$this->data);
	...
	public function add_product($time,$key)
	{
		...
		{	
			...
			redirect('superadmin/semantics/product_listing/'.id_encrypt($product_id));
		}
	...
	public function addEditProductImage($product_id='')
	{
		...
		$this->superAdminCustomView('retailer/product_managements/addEditSemanticProductImage',$this->data);
	}
	...
	public function product_listing($product_id)
	{
		...
		redirect('superadmin/semantics/product_detail/'.id_encrypt($product_id));
		}
		}
		...
		$this->superAdminCustomView('admin/semantics/test1',$this->data);
	}
	public function product_detail($product_id)
	{
		...
		if(isset($_POST) && !empty($_POST))
		{
			...
			if($this->form_validation->run())
			{
				...
				redirect('superadmin/semantics/addEditProductImage/'.id_encrypt($product_id));
		...
		$this->superAdminCustomView('admin/semantics/test2',$this->data);
	}
	public function deleteImage($productImageId='',$product_id='')
	{
		...
		redirect(base_url().'superadmin/semantics/addEditProductImage/'.$product_id);
	}
	...
	public function makeMainImage($productImageId='',$product_id='')
	{
		...
		redirect(base_url().'superadmin/semantics/addEditProductImage/addEditProductImage/'.id_encrypt($product_id));		
	}
	public function product_review($product_id)
	{
			...
			if((!empty($product_data->productTypeId))&&($product_data->productTypeId==3))
			{
				if(($this->session->userdata('userType')=='superadmin')||($this->session->userdata('userType')=='admin')||($this->session->userdata('userType')=='cse'))
				{
					...
					if($this->form_validation->run())
					{
						...
						redirect('superadmin/product_management');
					}
				}
				else
				{
		...
		$this->data['product_detail']   = $this->product_m->product_attributes_details($product_id);
		$this->superAdminCustomView('admin/semantics/test3',$this->data);
	}
