                        	...
                        	<div class="all-form-with">
								<div class="form-group">
                                	...
                                	<div class="col-sm-7 col-sm-7 padding_left_zero pd">
										...
										{
										?>
											<select class="form-control" name="level1" onchange="cat_list(this.value,2);" id="catLevel1">
										...
										{
										?>
											<select class="form-control" name="level1" id="catLevel1">
												<option value="">Select Category</option>
...
function ajax_search()
{
	...
	catId = $('option:selected:last').val();
	attNm = $('select:last').attr('name');
	expArr = attNm.split('level');
	
	if(catId!='')
	{
		nextLevel = parseInt(expArr[1])+1;
	}
	else
	{
		nextLevel = expArr[1];
		bkID = parseInt(nextLevel)-1;
		catId = $('#level'+bkID+' option:selected').val();
	}
	//console.log(catId+' '+nextLevel);
	if(catId!='')
	{
		$.ajax({
    		type: "POST",
	        url: '<?php echo base_url().$this->session->userdata('userType').'/free_shipping_product/level_category_list/'; ?>'+catId+'/'+nextLevel,
			data :'productName='+$('#productName').val()+'&categoryName='+$('#categoryName').val()+'&brandName='+$('#brandName').val()+'&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>',		
			dataType:'json',
	        beforeSend: function () {
