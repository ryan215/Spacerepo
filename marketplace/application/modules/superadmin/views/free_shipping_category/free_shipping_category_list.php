    	...
    	<div class="row">
        	<div class="col-md-12">
				<ul class="breadcrumbs-alt animated fadeInLeft">
					<li><a href="javascript:void(0);" class="current">Free Shipping Category Management</a></li>
				</ul>
			</div>
        </div>
		<div class="row">
			<div class="col-md-12" style="padding:0;">
            	<div class="col-lg-12">
                	<section class="panel" style="">
                    	...
                    	<header class="panel-heading panel-heading1">Free Shipping Category</header>
						<div class="panel-body" style="line-height:21px;">
							<section class="panel custom-panel" style="margin-bottom:0;">
									<a href="<?php echo base_url().'superadmin/free_shipping_category/add_free_shipping_cat'; ?>" class="btn btn-sm btn-shadow btn-success hvr-push" style="position:relative; top:-5px;">
...
{
	ajaxPage('<?php echo base_url().$this->session->userdata('userType').'/free_shipping_category/ajaxFun/'; ?>');
}

function ajaxPage(urlLink)
{	
	postData = 'sel_no_entry='+$('#sel_no_entry').val()+'&categoryName='+$('#categoryName').val();
	ajax_function(urlLink,'#ajaxData',postData);
}
ajaxPage('<?php echo base_url().$this->session->userdata('userType').'/free_shipping_category/ajaxFun/'; ?>');

function delete_free_cat(freeShipCatId)
{
	swal({   
	title: '',   
	...
	function(isConfirm){   
		if (isConfirm) 
		{     
			window.location.href = "<?php echo base_url().$this->session->userdata('userType').'/free_shipping_category/delete_free_shipping_category/'; ?>"+freeShipCatId; 
		} 
		else 
		{     
