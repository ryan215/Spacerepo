...
function check_false()
{
	...
	function(isConfirm){   
		if(isConfirm) 
		{     
			window.location.href = '<?php echo base_url().$this->session->userdata('userType').'/order_management/economic_change_false/'.$orderId; ?>';
		} 
		else 
		{     
...
function change_confirm_order_to_ready_order()
{
	...
	function(isConfirm){   
		if (isConfirm) 
		{     
			window.location.href = '<?php echo base_url().$this->session->userdata('userType').'/order_management/economic_change_confirm_order_to_ready_order/'.$orderId; ?>';
		} 
		else 
		{     
			swal("Cancelled","", "error");   
...
function delivery_print_invoice()
{
	$.ajax({
		type: "POST",
		url:'<?php echo base_url().$this->session->userdata('userType').'/order_management/economic_delivery_print_invoice'; ?>',
		data:'orderID=<?php echo $orderId; ?>&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>',
		success:function(result){
...
function delivery_print_page()
{
	$.ajax({
		type: "POST",
		url:'<?php echo base_url().$this->session->userdata('userType').'/order_management/economic_delivery_print_page'; ?>',
		data:'orderID=<?php echo $orderId; ?>&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>',
		success:function(result){
			var printWindow = window.open('', '','height=400,width=800');
				printWindow.document.write(result);
...
function change_ready_order_to_shipped_order()
{
	...
	function(isConfirm){   
		if (isConfirm) 
		{     
			window.location.href = '<?php echo base_url().$this->session->userdata('userType').'/order_management/economic_change_ready_order_to_shipped_order/'.$orderId; ?>';
		} 
		else 
		{     
			swal("Cancelled","", "error");   
...
function change_shipped_order_to_delivered_order()
{
	...
	function(isConfirm){   
		if (isConfirm) 
		{     
			window.location.href = '<?php echo base_url().$this->session->userdata('userType').'/order_management/economic_change_shipped_order_to_delivered_order/'.$orderId; ?>';
		} 
		else 
		{     
			swal("Cancelled","", "error");   
