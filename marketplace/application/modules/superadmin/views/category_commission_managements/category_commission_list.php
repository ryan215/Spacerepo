    	...
    	<div class="row">
      		<div class="col-lg-12">
        		<ul class="breadcrumbs-alt animated fadeInLeft">
          			<li>
                    	<a href="javascript:void(0);" class="current">Category Commission List</a>
                    </li>
                </ul>
      		</div>
<div class="col-lg-12">
	<section class="panel">
        ...
        <div class="panel-body">
        	<section class="panel custom-panel">
			   ...
			   <div style="padding:0;" class="col-lg-12">
			   		<h5>This commission is added after the retailer price is quoted.</h5>
			   </div>
			</section>
...
<section id="unseen" class="table-responsive">
	<table class="table table-invoice  table-hover table-striped table-custom table-search-head">
    	<thead>
        	<tr>
            	<th width="1%">S.No.</th>
                <th width="80%">Category Name<input type="hidden" class="form-control search table-head-search" id="categoryName" onkeyup="ajax_search();" placeholder="Category Name">
				</th>
                <th  width="20%">
                	Commission in %
                </th>
...
{
	ajaxPage('<?php echo base_url().$this->session->userdata('userType').'/category_commission_management/ajaxFun/'.$result['total']; ?>');
}
...
{	
	$.ajax({
		type: "POST",
		url:urlLink,
		data:'categoryName='+$('#categoryName').val()+'&sel_no_entry='+$('#sel_no_entry').val()+'&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>',
		beforeSend: function() {
