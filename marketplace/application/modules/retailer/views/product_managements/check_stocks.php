        ...
        <div class="row">
			<div class="col-md-12">
				<ul class="breadcrumbs-alt">
					...
					?>
					<li>
						<a href="<?php echo base_url().$this->session->userdata('userType').'/retailer_management'; ?>">Retailer List</a>
					</li>
					<li>
        	...
        	<div class="col-lg-12">
            	...
            	<section class="panel">
                	...
                	<div class="col-sm-12 panel-heading panel-heading1">
							...
							{
							?>
							<a href="<?php echo base_url().$this->session->userdata('userType').'/product_management/index/'.id_encrypt($organizationId); ?>" class="btn btn-sm btn-shadow btn-success hvr-push"><i class="fa fa-plus"></i>
								Add Product
							</a> 
							<?php
							}
							?>
											...
											<select class="form-control pull-left"  name='sel_no_entry' onchange="ajaxPage('<?php echo base_url().$this->session->userdata('userType').'/product_management/check_stocksAjaxFun/'.$result['total']; ?>');"  id='sel_no_entry' style="width:75px !important;display: inline-block;">
    	...
    	<table class="table table-invoice table-hover table-striped table-customm table-search-head">
        	<thead>
            	<tr>
					...
					<th width="20%">
                    	Product Name
                    	<input type="text" class="form-control search table-head-search" id="productName" onkeyup="ajaxPage('<?php echo base_url().$this->session->userdata('userType').'/product_management/check_stocksAjaxFun/'.$result['total']; ?>');" placeholder="Product Name">
                    </th>
...
ajaxPage('<?php echo base_url().$this->session->userdata('userType').'/product_management/check_stocksAjaxFun/'.$result['total']; ?>');
</script>