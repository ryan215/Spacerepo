						...
						<section>
							<div class="row">
								...
								<aside class="profile-info col-lg-12">
									<?php $this->load->view('admin/orders_managements/quick_shippment_order_details_view'); ?>
                        		</aside>
                			</div>
						</section>
					</div>
...
function check_false()
{
	...
	function(isConfirm){   
		if(isConfirm) 
		{     
			window.location.href = '<?php echo base_url().$this->session->userdata('userType').'/orders_management/quick_shippment_cancel_order/'.id_encrypt($orderDetailId); ?>';
		} 
		else 
		{     
...
function change_confirm_order_to_ready_order()
{
	...
	function(isConfirm){   
		if (isConfirm) 
		{     
			window.location.href = '<?php echo base_url().$this->session->userdata('userType').'/orders_management/quick_shippment_change_confirm_to_ready_order/'.id_encrypt($orderDetailId); ?>';
		} 
		else 
		{     
			swal("Cancelled","", "error");   
...
function delivery_print_invoice()
{
	$.ajax({
		type: "POST",
		url:'<?php echo base_url().$this->session->userdata('userType').'/orders_management/quick_shippment_print_invoice'; ?>',
...
function delivery_print_page()
{
	$.ajax({
		type: "POST",
		url:'<?php echo base_url().$this->session->userdata('userType').'/orders_management/quick_shippment_print_label'; ?>',
		data:'orderDetailId=<?php echo id_encrypt($orderDetailId); ?>&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>',
		success:function(result){
			var printWindow = window.open('', '','height=400,width=800');
				printWindow.document.write(result);
...
function change_ready_order_to_shipped_order()
{
	...
	function(isConfirm){   
		if (isConfirm) 
		{     
			window.location.href = '<?php echo base_url().$this->session->userdata('userType').'/orders_management/quick_shippment_change_ready_to_shipped_order/'.id_encrypt($orderDetailId); ?>';
		} 
		else 
		{     
			swal("Cancelled","", "error");   
...
function change_shipped_order_to_delivered_order()
{
	...
	function(isConfirm){   
		if (isConfirm) 
		{     
			window.location.href = '<?php echo base_url().$this->session->userdata('userType').'/orders_management/quick_shippment_change_shipped_to_delivered_order/'.id_encrypt($orderDetailId); ?>';
		} 
		else 
		{     
			swal("Cancelled","", "error");   
