<link href="<?php echo base_url() ?>css/admin/custom_admin.css" type="text/css" rel="stylesheet" />
<?php echo $this->load->view('upload_image_in_js'); ?>
<section id="main-content">
  <section class="wrapper"> 
    <!--contant start-->
    <div class="row">
    <div class="col-md-12">
      <ul class="breadcrumbs-alt animated fadeInLeft">
        <li class=""> <a href="<?php echo base_url().$this->session->userdata('userType').'/cse_management'; ?>"> CSE Management </a> </li>
        <li class=""> <a href="javascript:void(0);" class="current">Edit</a> </li>
      </ul>
    </div>
    <div class="col-lg-12">
      <section class="panel">
      <section class="panel custom-panel">
        <div class="col-lg-12 padding_left_zero padding_right_zero"> 
          <!--widget start-->
          <?php 
echo form_open();
?>
                                                                                              
            <section class="panel">
            <header class="panel-heading panel-heading1">Edit CSE</header>
            <div class="panel-body" >
              <center>
              <div class="details_main">
                <h2>CSE Details</h2>
                <img src="<?php echo base_url(); ?>images/frontend/user_icon.png" class="img-responsive"> <br />
                <div class="col-sm-12">
                  <div class="img_box">
                    <div class="right">
                     <?php
					if((!empty($result['imageName']))&&(file_exists('uploads/retailer/'.$result['imageName'])))											
					{
					?>
						<input type="hidden" name="imageName" id="hideImage" value="<?php echo $result['imageName']; ?>">
					<?php
					}
					else
					{
					?>
						<input type="hidden" name="imageName" id="hideImage">
					<?php
					}
					?>                	
                   <div id="uploadImage">
						<span id="imgname">
							<img class="img-circle" src="<?php echo base_url().'img/on-img.png'; ?>" height="100" width="100">
							
							
						</span>
                        
					</div>
                    <p style="color:#333; margin-top:4px;">Upload your display image</p>
                    </div>
                  </div>
                  <?php echo form_error('imageName');?> </div>
                <div class="col-lg-12 pd">
                  <div class="col-lg-6">
                    <div class="panel-body" style="line-height:21px;">
                      <div class="form-group text-left">
                        <div class="iconic-input cse-form-group right">
                          <label for="FirstName">First Name</label> <span class="error">*</span> <br />
                          <input type="text" id="FirstName" placeholder="First Name" name="firstName" class="form-control" value="<?php echo $result['firstName']; ?>">
                        </div>
                         <?php echo form_error('firstName');?>
                      </div>
                    </div>
                    </div>
                  <div class="col-lg-6">
                    <div class="panel-body" style="line-height:21px;">
                      <div class="form-group text-left">
                        <div class="iconic-input cse-form-group right">
                          <label for="MiddleName" class="pull-left">Middle Name</label><br />
                          <input type="text" id="MiddleName" placeholder="Middle Name" name="middleName" class="form-control" value="<?php echo $result['middleName'];?>">
                        </div>
                      </div>
                    </div>
                    <?php echo form_error('middleName');?> </div>
                </div>
                <div class="col-lg-6">
                  <div class="panel-body" style="line-height:21px;">
                    <div class="form-group text-left">
                      <div class="iconic-input cse-form-group right">
                        <label for="LastName" class="pull-left">Last Name</label>&nbsp;<span class="error">*</span> <br />
                        
                        <input type="text" id="LastName" placeholder="Last Name" name="lastName" class="form-control" value="<?php echo $result['lastName']; ?>">
                      </div>
                       <?php echo form_error('lastName');?> 
                    </div>
                  </div>
                 </div>
                <div class="clearfix"></div>
                <h2>DOB Details</h2>
                <img src="<?php echo base_url(); ?>images/dob_icon.png" class="img-responsive">
                <div class="col-lg-6">
                  <div class="panel-body" style="line-height:21px;">
                    <div class="form-group text-left">
                        <div class="iconic-input cse-form-group right">
                          <label for="Date" class="pull-left">Date</label>
                          <select class="form-control selectpicker" data-live-search="true" id="Date" name="date">
                            <option value="">Date</option>
                            <?php 
										for($i=1;$i<=31;$i++)
												{
												?>
                            <option value="<?php echo $i; ?>" <?php if($result['date']==$i){ ?> selected="selected" <?php } ?>> <?php echo $i; ?> </option>
                            <?php
												}													
												?>
                          </select>
                        </div>
                        <?php echo form_error('date'); ?> 
                      </div>
                  </div>
                </div>
                <div class="col-lg-6">
                  <div class="panel-body" style="line-height:21px;">
                  	<div class="form-group text-left">
                    	  <div class="iconic-input right">
                      <?php $calender   = cal_info(0);
											$month_list = $calender['months'];?>
                      <label for="Month" class="pull-left">Month</label>
                      <select class="form-control selectpicker" name="month" id="Month" data-live-search="true">
                        <option value="" > Month</option>
                        <?php
											for($i=1;$i<=12;$i++)
											{
											?>
                        <option value="<?php echo $i; ?>" <?php if($result['month']==$i){ ?> selected="selected" <?php } ?>> <?php echo $month_list[$i]; ?> </option>
                        <?php
											}
											?>
                      </select>
                    </div>
                             <?php echo form_error('month'); ?>
                    </div>
                  </div>
          </div>
                <h2>Address Details</h2>
                <img src="<?php echo base_url(); ?>images/frontend/location_icon.png" class="img-responsive">
                <?php /*?><div class="col-lg-6">
                  <div class="panel-body" style="line-height:21px;">
                    <div class="form-group text-left">
                      <div class="iconic-input right">
                        <label for="country" class="pull-left">Country</label>
                        <select name="countryId" onchange="state_list(this.value)" class="form-control selectpicker" data-live-search="true">
                          <option value="">Select Country</option>
                          <?php 
									if(!empty($result['countryList']))
									{
										foreach($result['countryList'] as $row)
										{
									?>
                          <option value="<?php echo $row->countryId; ?>" <?php if($result['countryId']==$row->countryId){ ?> selected="selected" <?php } ?>> <?php echo $row->name; ?> </option>
                          <?php	
										}
									}
									?>
                        </select>
                        <?php echo form_error('countryId'); ?> </div>
                    </div>
                  </div>
                </div><?php */?>
                <div class="col-lg-6">
                  <div class="panel-body" style="line-height:21px;">
                    <div class="form-group cse-form-group">
                     	
                        	 <label for="State" class="">State</label><br />
                      <div class="iconic-input right" id="stateList">
                        <select class="form-control selectpicker" data-live-search="true" name="stateId" onchange="city_list(this.value);">
                           <option value="">Select State</option>
                          <?php 
									if(!empty($result['stateList']))
									{
										foreach($result['stateList'] as $row)
										{
									?>
                          <option value="<?php echo $row->stateId; ?>" <?php if($result['stateId']==$row->stateId){ ?> selected="selected" <?php } ?>> <?php echo $row->stateName; ?> </option>
                          <?php	
										}
									}
									?>
                        </select>
                
                      <?php echo form_error('stateId'); ?> </div>
                        </div>
                  </div>
                </div>
				<div class="col-lg-6">
                  <div class="panel-body" style="line-height:21px;">
                    <div class="form-group cse-form-group">
                     	
                        	 <label for="State" class="">Area</label><br />
                      <div class="iconic-input right" id="cityList">
                        <select class="form-control selectpicker" data-live-search="true" name="cityId">
                           <option value="">Select Area</option>
                        </select>
                		 </div>
						 <?php echo form_error('cityId'); ?>
                      </div>
                  </div>
                </div>
                <div class="clearfix"></div>
              </div>
            </div>
            </center>
            <br />
            <br />
            <div class="col-sm-12 ">
              <center>
                <a class="btn btn-danger btn-save" href="<?php echo base_url().$this->session->userdata('userType').'/cse_management'; ?>"> Cancel </a> &nbsp;&nbsp;
                <button class="btn btn-success btn-save">Save</button>
              </center>
            </div>
          </form>
        </div>
      </section>
      <!--widget end--> 
    </div>
  </section>
</section>
</div>
</div>
<!--contant end-->
</section>
</section>
<script type="text/javascript">
$( document ).ajaxComplete(function() {	
	$('.selectpicker').selectpicker();
});

function city_list(stateId)
{	
	$.ajax({
		type: "POST",
		url:'<?php echo base_url().$this->session->userdata('userType').'/location_management/cityStateList'; ?>',
		data:'stateId='+stateId+'&cityId=<?php echo $result['cityId']; ?>&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>',
		beforeSend: function() {
			$('#cityList').html('<?php echo $this->loader; ?>');
		},
		success:function(result){  
			$('#cityList').html(result);	
		}
	});
}

<?php
if($result['stateId'])
{
?>
	city_list('<?php echo $result['stateId']; ?>');
<?php
}
?>
</script>
<style>
.ajax-upload-dragdrop{/* width:78% !Important;*/ padding:0px !Important; width:100px !important; text-align:center;}

.upload-statusbar{border:0; 
}

.upload-red {font-size:12px; padding:3px 15px;
}

.ajax-upload-dragdrop input{padding-left:0 !important;
	padding-right:0 !important;
	height:100px !important;
}

.ajax-upload-dragdrop  .upload-statusbar{margin-left:-120px !important;
}

.browse-star{ color: #ff6c60;
    font-size: 6px;
    line-height: 18px;
	margin-bottom: -8px;
    margin-right: -80px;
}
input[type=radio] { display:block !Important;}

.bootstrap-select.btn-group .dropdown-menu{max-height:250px !important;
}


.bootstrap-select .btn-default{background-color: #fff !important;
    border-color: #e2e2e4 !important;1px solid 
    color: #666 !important;
}

.bootstrap-select .btn-default span{color:#666;
}

.dropdown-menu{max-height:225px !important;
}


</style>
