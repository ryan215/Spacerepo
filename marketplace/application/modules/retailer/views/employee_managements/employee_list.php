                    	...
                    	<section class="panel custom-panel margin-btm-zero">
							...
							<div class="col-sm-5 " style="padding: 5px;">
								 <div class="col-sm-2 col-lg-2" style="padding-left:0px;">
								 	<a class="btn btn-sm btn-shadow btn-success hvr-push" type="button" href="<?php echo base_url().$this->session->userdata('userType').'/employee_management/addEmployee'; ?>">
                        	...
                        	<table class="table table-invoice table-custom table-search-head">
								<thead>
									<tr>
										...
										<th>
											Name
											<input type="text" class="form-control search table-head-search" id="employeeName" onkeyup="ajax_search();" placeholder="Name">
										</th>
...
{
	ajaxPage('<?php echo base_url().$this->session->userdata('userType').'/employee_management/ajaxFun/'; ?>');
}
...
function ajaxPage(urlLink)
{
	if($("#employeeName").val())
	{
		$("#employeeName").css('width','98%');
		$("#employeeName").css('background','white');
	}
	else
	{
		$("#cseName").css('width','');
	...
	postData = 'employeeName='+$('#employeeName').val()+'&sel_no_entry='+$('#sel_no_entry').val();
