...
if(!empty($cartDetails))
{
	foreach($cartDetails as $row)
	{
		...
		{
			$pickupProccessPrice = $pickupProccessPrice+($row->quantity*$row->pickupProccessPrice);
		}
		$totalPickupProcsFee = $totalPickupProcsFee+$pickupProccessPrice;
		$totalAmt = $totalAmt+($row->quantity*$displayPrice)+$pickupProccessPrice;
?>
	<tr class="first odd">
    	<td class="image"><a class="product-image" title="<?php echo $row->code; ?>" href="<?php echo base_url().'frontend/single/product_detail/'.id_encrypt($row->productId); ?>">
        	<?php
        ...
        <td>
        	<h2 class="product-name" style="margin-bottom:5px;"> 
            	<a href="<?php echo base_url().'frontend/single/product_detail/'.id_encrypt($row->productId); ?>"><?php echo $row->code; ?></a> 
            </h2>
            <?php
            ...
            <span style="margin-bottom:5px;">
                      ...
                      <a class="btn  btn-xs color_box active_color color_static" style="background-color:<?php echo $row->colorCode; ?>" href="javascript:void(0);"></a> </span>
                      <?php
						}
						?>
                      <?php
                      ...
                      <a class="btn btn-xs size_box  active_color size_static" href="javascript:void(0);"><?php echo $row->size; ?></a> </span>
                      <?php
						}
						?>
                      <span class="item-info">&nbsp;</span><br>
                      <span class="seller-name"> Seller: <span> <a href="javascript:void(0);"><strong> <?php echo $row->organizationName; ?> </strong></a> </span> </span></td>
                    <td class="a-center"><span class="cart-price"> <span class="price">₦<?php echo number_format($displayPrice,2); ?></span> </span></td>
                    <td class="a-center movewishlist"><?php echo $row->quantity; ?></td>
					<td class="a-center movewishlist">
    	<span class=""> 
        	<span class="">
				&#x20A6;<?php echo number_format($row->pickupProccessPrice,2); ?>
            </span> 
		</span>
	</td>
                    <td class="a-center movewishlist"><span class="cart-price"> <span class="price">₦<?php echo number_format($displayPrice*$row->quantity,2); ?></span> </span></td>
						...
						<div class="col-sm-12" style="display:inline-block;">
							<div class="shipping_method_shows" style="width:100%;float:left;">
								...
								<div class="col-sm-6">
									<strong>
										<?php
										if($totalPickupProcsFee)
										{
											echo '&#8358;'.number_format($totalPickupProcsFee,2);
										}
										else
										{
											echo 'Free';
