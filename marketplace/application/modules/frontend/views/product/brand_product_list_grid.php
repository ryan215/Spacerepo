	...
	<div class="container main-contant-wrapper shadow-main-div">
  		<div class="row">
			<div class="yt-breadcrumbs">
    			...
    			<ul><li class="home" itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb"><a itemprop="url" href="<?php echo base_url(); ?>" title="Go to Home Page"><span itemprop="title">Home</span></a></li><li class="category4" itemscope="" itemtype=""><strong><?php echo $brandName; ?></strong></li></ul>
					</div>
        			</div>
        		</div>
        	</div>
			...
			<div class="col-main col-sm-9 col-sm-push-3">
				<article class="col-main col-sm-12 product-listmain-container">
					...
					<div class="toolbar col-sm-12" style="padding-top:0px;">
						<div class="sorter">
							<div class="view-mode">
								<a href="<?php echo frontend_grid_brand_url($brandId,$brandName); ?>" title="Grid" class="button-list active" style="background-color:#fe5621; color:white;" id="grid">
									Grid
								</a> &nbsp;&nbsp;
								<a href="<?php echo frontend_list_brand_url($brandId,$brandName); ?>" title="List" class="button-list">
									List
								</a> 
							</div>
						</div>
					 ...
					 <div  class="col-sm-12 pd" style="padding:0;"> 
						<div class="category-products col-sm-12" style="padding-right:0px;">
									...
									<div align="center" style="width:97.2%;">
										...
										<div id="loadMore" class="load_more col-sm-12" style="display:none; cursor:pointer; padding-left:0px;">
										</div>
						...
						<div id="" class="super-category-block first-load sn-category-block">
                                        <div class="block-title-defaults ">
                                            <div class="tab-category-title block-title">
                                                <strong><span style="font-size:14px !Important"><?php echo $brandName; ?></span></strong>
...
$(document).ready(function() {
	...
	$.ajax({
		type: "POST",
		url:'<?php echo base_url().'frontend/product/brandAjaxFunGrid'; ?>',
		data:'page='+track_click+'&brandId=<?php echo $brandId; ?>&pSorting='+pSorting+'&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>',
		dataType: 'json',
		beforeSend: function() {
			$('#results').html('<div style="bottom: 0;left: 0; position: absolute; right: 0;top: 220px;"><img src="<?php echo base_url(); ?>images/frontend/circle-simple_light.gif"></div>');
		},
	...
	$(".load_more").click(function (e) { //user clicks on button
        ...
        {	//alert(track_click+' < '+total_pages);
			pSorting = $('#pSorting').val();
			var posdata = $("form#srchFrm").serialize()+'&page='+track_click+'&brandId=<?php echo $brandId; ?>&pSorting='+pSorting;
			$.ajax({
				type: "POST",
				url:'<?php echo base_url().'frontend/product/brandAjaxFunGrid'; ?>',
...
{	
	...
	track_click = 0;
	posdata = jQuery("form#srchFrm").serialize()+'&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>&page='+track_click+'&brandId=<?php echo $brandId; ?>&pSorting='+pSorting;
	jQuery.ajax({
		type: "POST",
		url:'<?php echo base_url().'frontend/product/brandAjaxFunGrid'; ?>',
		data:posdata,
...
function searchfun()
{
	...
	$("#thank").css('display','none');
	track_click = 0;
	posdata = $("form#srchFrm").serialize()+'&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>&page='+track_click+'&brandId=<?php echo $brandId; ?>&pSorting='+pSorting;
	$.ajax({
		type: "POST",
		url:'<?php echo base_url().'frontend/product/brandAjaxFunGrid'; ?>',
		data:posdata,
