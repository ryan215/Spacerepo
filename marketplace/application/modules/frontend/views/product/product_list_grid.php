			...
			<div class="col-main col-sm-9 col-sm-push-3">
				<article class="col-main col-sm-12 product-listmain-container">
					...
					<div class="toolbar col-sm-12" style="padding-top:0px;">
						<div class="sorter">
							<div class="view-mode">
								<a href="<?php echo frontend_grid_category_url($categoryId,$categoryName); ?>" title="Grid" class="button-list active" style="background-color:#fe5621; color:white;" id="grid">
									Grid
								</a> &nbsp;&nbsp;
								<a href="<?php echo frontend_list_category_url($categoryId,$categoryName); ?>" title="List" class="button-list">
									List
								</a> 
							</div>
						</div>
...
$(document).ready(function() {
	...
	$.ajax({
		type: "POST",
		url:'<?php echo base_url().'frontend/product/ajaxFunGrid'; ?>',
		data:'page='+track_click+'&categoryId=<?php echo $categoryId; ?>&pSorting='+pSorting+'&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>',
		dataType: 'json',
		beforeSend: function() {
	...
	$(".load_more").click(function (e) { //user clicks on button
        ...
        {	//alert(track_click+' < '+total_pages);
			pSorting = $('#pSorting').val();
			var posdata = $("form#srchFrm").serialize()+'&page='+track_click+'&categoryId=<?php echo $categoryId; ?>&pSorting='+pSorting;
			$.ajax({
				type: "POST",
				url:'<?php echo base_url().'frontend/product/ajaxFunGrid'; ?>',
...
{	
	...
	track_click = 0;
	posdata = jQuery("form#srchFrm").serialize()+'&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>&page='+track_click+'&categoryId=<?php echo $categoryId; ?>&pSorting='+pSorting;
	jQuery.ajax({
		type: "POST",
		url:'<?php echo base_url().'frontend/product/ajaxFunGrid'; ?>',
		data:posdata,
...
function searchfun()
{
	...
	$("#thank").css('display','none');
	track_click = 0;
	posdata = $("form#srchFrm").serialize()+'&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>&page='+track_click+'&categoryId=<?php echo $categoryId; ?>&pSorting='+pSorting;
	$.ajax({
		type: "POST",
		url:'<?php echo base_url().'frontend/product/ajaxFunGrid'; ?>',
		data:posdata,
