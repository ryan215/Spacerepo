					...
					<fieldset>
						<table class="data-table cart-table" id="shopping-cart-table">
							...
							<tbody>
								...
								if(!empty($result))
								{
									$displayPrice  = $result->productAmt;
									$productWeight = $result->productWeight;
									$subTotal	   = $result->quantity*$displayPrice;	
									$productId 	   = $result->productId;						
									$totalAmt  	   = $totalAmt+$subTotal+$shipping;
								?>
<tr class="first odd">
	<td class="image">
		<?php
		...
		{
		?>
			<a class="product-image" title="<?php echo $result->code; ?>" href="<?php echo base_url().'frontend/single/product_detail/'.id_encrypt($result->productId); ?>">
				<img src="<?php echo base_url().'uploads/product/thumb500_500/'.$result->imageName; ?>" width="75" height="75"/>
			</a>
		<?php
		}
		...
		?>
			<a class="product-image" title="<?php echo $result->code; ?>" href="<?php echo base_url().'frontend/single/product_detail/'.id_encrypt($result->productId); ?>">
				<img src="<?php echo base_url().'uploads/product/'.$result->imageName; ?>" width="75" height="75"/>
			</a>
		<?php
		}
		else
    ...
    <td>
		<h2 class="product-name" style="padding-bottom:5px;">
			<a href="<?php echo base_url().'frontend/single/product_detail/'.id_encrypt($result->productId); ?>">
		...
		<span style="padding-bottom:5px;">
            ...
            <a class="btn  btn-xs color_box active_color color_static" style="background-color:<?php echo $result->colorCode; ?>" href="javascript:void(0);"></a>
		</span>
		<?php
		}
		...
		{
		?>
		<span class="" style="padding-bottom:5px;"> 
			<label class="">Size : </label>
            <a class="btn btn-xs size_box  active_color size_static"  href="javascript:void(0);"><?php echo $result->size; ?></a>
		</span>
		<?php
		}
		?>
		...
		<span class="seller-name">Seller: 
			<span>
				<a href="javascript:void(0);"><strong><?php echo $result->organizationName; ?></strong></a>
			</span>
		</span>
	</td>
    <td class="a-right">
    ...
    <td class="a-left movewishlist">
    	<div class="qty-div">
			<a href="javascript:void(0)" onclick="qty_decryment('<?php echo $result->cartId; ?>');"><div>-</div></a>
			<div id="qty<?php echo $result->cartId; ?>"><?php echo $result->quantity; ?></div>
			<a href="javascript:void(0)" onclick="qty_increment('<?php echo $result->cartId; ?>');"><div>+</div></a>
		</div>
    </td>   
    <td class="a-right movewishlist">
		<span class="cart-price"> 
			<span class="price">
				&#x20A6;<span id="subtotal<?php echo $result->cartId; ?>">
    ...
    <td class="a-center last">
		<a href="javascript:void(0);" class="button remove-item" title="Remove item" onClick="remove_to_cart('<?php echo id_encrypt($result->cartId); ?>');">
			<span><span>Remove item</span></span>
							...
							<tfoot>
								<tr class="first last">
									...
									?>
			                        <a id="empty_cart_button" class="button btn btn-empty button_carts" title="Clear Cart" value="empty_cart" name="update_cart_action" type="submit" onclick="return clear_cart('<?php echo id_encrypt($result->cartId); ?>');"><span><span>Clear Cart</span></span></a>										
				                  </tr>
			                </tfoot>
							<?php
							}
					...
					<div class="totals col-sm-4">
						...
						<div class="inner">
							...
							<ul class="checkout">
								...
								{
								?>
									<a class="button btn btn-proceed-checkout" title="Proceed to Checkout" type="button" href="<?php echo base_url().'frontend/product_buy_now/shipping_address/'.id_encrypt($cartId); ?>" style="padding: 8px 15px;">
...
function qty_increment(cartId)
{
	$.ajax({
		type: "POST",
		url:'<?php echo base_url(); ?>frontend/product_buy_now/qty_increment',
...
function qty_decryment(cartId)
{
	...
	if(qtnty>1)
	{
		$.ajax({
		type: "POST",
		url:'<?php echo base_url(); ?>frontend/product_buy_now/qty_decryment',
		data:'cartId='+cartId+'&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>',
		dataType: 'json',
		success:function(result){ 	
			if(result.success==1)
			{
				if(result.message==0)
				{
					window.location.href = '<?php echo base_url().'frontend/product_buy_now/buy_now_page/'.id_encrypt($cartId); ?>';
				}
				else
				{
					$('#qty'+cartId).text(result.message);
	...
	function(isConfirm){   
		...
		{   
			$.ajax({
				...
				url:'<?php echo base_url(); ?>frontend/product_buy_now/remove_to_cart/'+cartId,
				success:function(result){
