                  ...
                  <div id="collapseOne" class="panel-collapse collapse in" style="overflow:visible; margin-top:0px; padding:0px;">
                    <div class="panel-body shipping_sec" style="width:100%;  height: 65px;  margin-top: 0px !Important;  padding-top: 20px;">
                      ...
                      <div class="col-sm-6 padding_right_zero">
                        ...
                        <a data-toggle="modal" data-target="#modal-delivery" class="account-toplink pull-right button btn-addnew" title="Add New Delivery Address" <?php if((!empty($shippCusAddId))&&($shippCusAddId)){?> style="display:none;"<?php } ?> id="addDelAdd"> <i class="fa fa-plus"></i> &nbsp;Add New Delivery Address </a>
...
?>
	...
	if(!empty($allShippCusAdd))
	{
                      ...
                      <div class="shipping_add_box <?php if($row->addressId==$shippCusAddId){ echo 'even'; }else{ echo 'odd'; } ?>" style="width:48%;float:left;">
                        ...
                        <a href="<?php echo base_url().'frontend/product_buy_now/check_shipping_delivery_here/'.id_encrypt($cartId).'/'.id_encrypt($row->addressId); ?>" class="button btn-delivery btn-disable">Deliver Here</a>
                        </div>
                      </div>
                      <?php
					}
	...
	?>
        ...
        <div class="panel panel-default">
			<a href="<?php echo base_url().'frontend/product_buy_now/pickup_address/'.id_encrypt($cartId); ?>">
				<div class="panel-heading">
                  ...
                  <div id="collapseTwo" class="panel-collapse collapse">
					...
					<div id="productList" style="display:none; margin-bottom:20px;">
						<div class="super-category-block first-load sn-category-block" style="margin-bottom:0;">
                    		<div class="block-title-defaults ">
...
if(!empty($cartDetails))
{
	...
	elseif($cartDetails->genuineShippFee)
	{
		...
		if(!empty($shipRateDet))
		{
			...
			$economicalShippAmt = $shippingRate;
		}
						
		$shippingRate = $cartDetails->shippingRate;
		if($productWeight>10)
		{
			$shippingRate = $cartDetails->shippingRate*$productWeight;
		}
		$shipping_rate    = $shipping_rate+($cartDetails->quantity*$shippingRate);
		$standardShippAmt = $standardShippAmt+($cartDetails->quantity*$shippingRate);
	}
	$totalAmt = $totalAmt+($cartDetails->quantity*$displayPrice);
?>
	<tr class="">
    	<td class="image">
		...
		<td>
			...
			<span class="seller-name">
				Seller: <span>
					...
					<?php echo $cartDetails->organizationName; ?>
						</strong>
                	</a>
				</span>
			</span>
        ...
        <?php
					?>
		<td class="a-center movewishlist"><span class="cart-price"> <span class="price">&#x20A6;<?php echo number_format($displayPrice*$cartDetails->quantity,2); ?></span> </span>
        </td>
	</tr>
	<?php		
	}
	$totalProduct = $totalAmt+$shipping_rate;
?>
					</tbody>
				</table>
        	</fieldset>
					...
					<div id="capthImg" style="display:none; border:1px solid #ccc; border-top:0 !important;">
						<div class="col-sm-12 captcha-maindiv">
							...
							<input type="hidden" name="economicCapcha" id="captchaHidden" value="0" />
							</form>
						</div>
            			</div>
					</div>
            		...
            		<fieldset>
						<table class="data-table cart-table cart-table-customnew" id="shopping-cart-table">
                			...
                			<tbody>
								...
								if(!empty($cartDetails))
								{
									$etaDay = $cartDetails->ETA+$this->config->item('estimated_time_increase');
									$minEta = $etaDay;
									$maxEta = $etaDay;
...
<tr class="first last">
    ...
    <td class="a-right">
		<span class="cart-price"> <span class="price">
    		<?php 
			echo '&#8358;'.number_format(($cartDetails->productAmt*$cartDetails->quantity),2); 
			?>
         </span> </span>
	</td>
    <td class="a-center movewishlist"><div class="qty-div"><?php echo $cartDetails->quantity; ?></div></td>
</tr>
	<?php
	$displayPrice  = $cartDetails->productAmt;
	$productWeight = $cartDetails->productWeight;
...
?>
				...
				{
				?>
					<div class="chk_main_div" id="rightShippingAdd">
						<div id="" class="super-category-block first-load sn-category-block" style="margin-bottom:0px;">
							<div class="block-title-defaults ">
								<div class="tab-category-title block-title">
				...
				<div class="chk_main_div" id="rightShippingRate" style="display:none;">
					<div id="" class="super-category-block first-load sn-category-block" style="margin-bottom:0px;">
						<div class="block-title-defaults ">
							<div class="tab-category-title block-title">
				...
				<!--<a href="#" data-toggle="tooltip" data-original-title="Note: All of the products in the cart will be delivered separately" ><i class="fa fa-question-circle"></i></a>-->
                ...
                <div style="font-style:italic; color:#FE5621; font-size:14px;">
					...
					?>
							...
							<?php
							if((!empty($shippCusDet))&&($shippCusDet->state)&&(!empty($cartDetails->dropshipStateId))&&($cartDetails->dropshipStateId==$shippCusDet->state))
							{
								$time = date('H');
								if((!empty($totalWeight))&&($totalWeight)&&($totalWeight<$this->config->item('flate_weight')))
								{
									if($time<$this->config->item('same_day_delivery_time'))
									{
										$sameDayDeliveryOpt = 1;
									?>
									<div>								
					...
					{
					?>
            <a title="Place Order Now" class="button btn-placeorder" onclick="select_shipping_add();" style="cursor:pointer;"> <span><span style="font-size: 18px !important;">Place Order Now</span></span> </a>
            		<?php
					}						
					?>
				</div>
...
{	
	...
	$.ajax({
		type: "POST",
		url:'<?php echo base_url().'frontend/product_buy_now/economical_shipping_amount/'.id_encrypt($cartId); ?>',
		data:'<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>',
		dataType:'json',
...
function standard_fun()
{
	...
	$.ajax({
		type: "POST",
		url:'<?php echo base_url().'frontend/product_buy_now/standard_shipping_amount/'.id_encrypt($cartId); ?>',
		data:'<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>',
		dataType:'json',
		success:function(result){ 	//alert(result);
			$('div#deleveryBtn').css('display','block');
...
function sameday_fun()
{
	...
	$.ajax({
		type: "POST",
		url:'<?php echo base_url().'frontend/product_buy_now/same_day_delivery_shipping_amount/'.id_encrypt($cartId); ?>',
		data:'<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>',
		dataType:'json',
...
function economic_cash_on_delivery()
{
	...
	$.ajax({
		type: "POST",
		url:'<?php echo base_url().'frontend/product_buy_now/economic_cash_handling_amount/'.id_encrypt($cartId); ?>',
		data:'<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>',
		dataType:'json',
		success:function(result){ 
			if(result.success==1)
			{	
				...
				$('#captchaHidden').val(1);
				$('div#payCashBtn').html('<a title="Place Order Now" class="button btn btn-placeorder-sub" onclick="confirm_order();"  style="line-height:51px; margin-right:0; border:0; color:#fff;"><span style="font-size: 18px !important;">Confirm Order</span></a>');
			}			
		}
	});
}
...
function standard_cash_on_delivery()
{
	...
	$.ajax({
		type: "POST",
		url:'<?php echo base_url().'frontend/product_buy_now/standard_cash_handling_amount/'.id_encrypt($cartId); ?>',
		data:'<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>',
		dataType:'json',
		success:function(result){ 
			if(result.success==1)
			{	
				...
				$('strong#cashHandlingRate').html(result.cashHandlingAmt);
				$('#captchaHidden').val(2);
				$('div#payCashBtn').html('<a title="Place Order Now" class="button btn btn-placeorder-sub" onclick="confirm_order();"  style="line-height:51px; margin-right:0; border:0; color:#fff;"><span><span style="font-size: 18px !important;">Confirm Order</span></span></a>');
			}			
		}
	});
}
...
function same_day_cash_on_delivery()
{
	...
	$.ajax({
		type: "POST",
		url:'<?php echo base_url().'frontend/product_buy_now/same_day_delivery_cash_handling_amount/'.id_encrypt($cartId); ?>',
		data:'<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>',
		dataType:'json',
		success:function(result){ 
			if(result.success==1)
...
if(!empty($shippCusAddId))
{
	...
	elseif((!empty($sameDayDeliveryOpt))&&($sameDayDeliveryOpt))
	{
	}
	else
	{
