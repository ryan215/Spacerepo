	...
	<div class="container main-contant-wrapper shadow-main-div">
  		<div class="row">
			<div class="yt-breadcrumbs">
    			...
    			<ul><li class="home" itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb"><a itemprop="url" href="<?php echo base_url(); ?>" title="Go to Home Page"><span itemprop="title">Home</span></a></li><li class="category4" itemscope="" itemtype=""><strong><?php echo $organizationName; ?></strong></li></ul>
					</div>
        			</div>
        		</div>
        	</div>	
			<div class="col-main col-sm-9 col-sm-push-3">
				<article class="col-main col-sm-12 product-listmain-container">
					...
					<div class="toolbar col-sm-12" style="padding-top:0px;">
						<div class="sorter">
							<div class="view-mode">
								<a href="<?php echo base_url();?>frontend/seller/seller_product/<?php echo $sellerId;?>" title="Grid" class="button-list">
									Grid
								</a> &nbsp;&nbsp;
								<a href="<?php echo base_url();?>frontendseller/seller_product_list/<?php echo $sellerId;?>" title="List" class="button-list active" style="background-color:#fe5621; color:white;" id="list">
									List
								</a> 
							</div>
						</div>
...
$(document).ready(function() {
	...
	$.ajax({
		type: "POST",
		url:'<?php echo base_url().'frontend/seller/ajaxFunList'; ?>',
	...
	$(".load_more").click(function (e) { //user clicks on button
        ...
        {	//alert(track_click+' < '+total_pages);
			pSorting = $('#pSorting').val();
			var posdata = $("form#srchFrm").serialize()+'&sellerId=<?php echo $sellerId; ?>&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>&page='+track_click+'&pSorting='+pSorting;
			$.ajax({
				type: "POST",
				url:'<?php echo base_url().'frontend/seller/ajaxFunList'; ?>',
				data:posdata,
...
function searchfun()
{
	...
	track_click = 0;
	posdata = $("form#srchFrm").serialize()+'&sellerId=<?php echo $sellerId; ?>&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>&page='+track_click+'&pSorting='+pSorting;
	$.ajax({
		type: "POST",
		url:'<?php echo base_url().'frontend/seller/ajaxFunList'; ?>',
		data:posdata,
...
function brandSearchFun(brndNm)
{
	...
	posdata = 'brandName='+brndNm+'&sellerId=<?php echo $sellerId; ?>&brandList=<?php echo json_encode($brandList); ?>&<?php echo $this->security->get_csrf_token_name(); ?>=<?php echo $this->security->get_csrf_hash(); ?>';
	$.ajax({
		type: "POST",
