				...
				if(!empty($orderTotalId))
				{
					...
					if((!empty($orderTotal['addressDet']))&&($orderTotal['addressDet']))
					{
						if($orderTotal['isPickup'])
						{
							...
							{
								 $this->db->select('address.addressId');
								...
								$this->db->where(array('customer.customerId' => $orderTotal['customerId'],'customer_address.customerAddressId' => 1));
								$resultsAdd = $this->db->get()->row();
								if(!empty($resultsAdd))
								{
									$insertOpt8 = array(
														'orderTotalId'   => $orderTotalId,
														'addressId'      => $resultsAdd->addressId,
														'addressTypeId'  => 1,
														'active' 		 => 1,
														'createDt'		        	=> $orderTotal['createDt'],
														'createBy'			    	=> $orderTotal['createBy'],
